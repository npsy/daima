...
% ========== 稀疏角修改：每隔 3 个投影取 1 个 ==========
step = 3;                              % 降采样步长
p = p(:, 1:step:end);    % 等间隔抽取
views = size(p, 2);             % 更新 views
% ====================================================
...
%% ============================================================
% SART Reconstruction (with EBPN framework)
%% ============================================================
fprintf('\nStarting SART Reconstruction...\n');
...
while ebpn < max_EBPN
    ...
    % ------ 一次 SART 迭代（完整投影遍历 = 1次等效反投影）------
    img_new = SART(img, p, 1);   % nIter=1，遍历所有投影一次
    ...
    if ismember(ebpn, [1,5,10,20,50,100,200])
        save(sprintf('SART_EBPN_%d.mat', ebpn), 'img_new');
    end
...
colormap gray;
colorbar;
title(sprintf('Best SART (EBPN=%d)', best_iter));
...
save('SART_Result_SparseView.mat', 'Result');
