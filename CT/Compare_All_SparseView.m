...
%% ================= 1. 加载所有稀疏角结果 =================
load('EM_Result_SparseView.mat');      % 有限角 EM 结果
Result_EM = Result;

load('OSEM_Result_SparseView.mat');
Result_OSEM = Result;

load('SART_Result_SparseView.mat');
Result_SART = Result;

load('FBP_Result_SparseView.mat', 'Result');
Result_FBP = Result;
...
ylabel('PSNR (dB)');
title('PSNR Convergence Comparison (Sparse View)');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'PSNR_vs_EBPN_SparseView.png', '-dpng', '-r300');
...
xlabel('Time (seconds)');
ylabel('PSNR (dB)');
title('PSNR vs. Running Time (Sparse View)');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'PSNR_vs_Time_SparseView.png', '-dpng', '-r300');
...
xlabel('Equivalent Backprojection Number');
ylabel('SSIM');
title('SSIM Convergence Comparison (Sparse View)');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'SSIM_vs_EBPN_SparseView.png', '-dpng', '-r300');
...
xlabel('Time (seconds)');
ylabel('SSIM');
title('SSIM vs. Running Time (Sparse View)');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'SSIM_vs_Time_SparseView.png', '-dpng', '-r300');
...
% --- 自动保存为 PNG ---
print(gcf, 'Best_Images_and_Errors_SparseView.png', '-dpng', '-r300');

%% ================= 8. 运行时间对比 =================
fprintf('========== Runtime Comparison (max EBPN = 200, Sparse View) ==========\n');
fprintf('EM    : %.2f sec\n', Result_EM.runtime);
...
%% ================= 9. 达到特定 PSNR 所需 EBPN =================
% 稀疏角下 PSNR 介于全采样与有限角之间，建议设为 15 dB
target_psnr = 15;   % 根据你的实际数据调整
...
fprintf('\n--- EBPN required to reach PSNR = %.1f dB (Sparse View) ---\n', target_psnr);
fprintf('EM    : %.1f\n', ebpn_em_target);
fprintf('OS-EM : %.1f\n', ebpn_osem_target);
fprintf('SART  : %.1f\n', ebpn_sart_target);
fprintf('FBP   : 1 (fixed)\n');