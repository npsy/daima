...
%% ================= 1. 加载所有结果 =================
load('EM_Result.mat');      % 包含 Result
Result_EM = Result;

load('OSEM_Result.mat');
Result_OSEM = Result;

load('SART_Result.mat');
Result_SART = Result;

load('FBP_Result.mat', 'Result');
Result_FBP = Result;
...
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'PSNR_vs_EBPN.png', '-dpng', '-r300');
...
xlabel('Time (seconds)');
ylabel('PSNR (dB)');
title('PSNR vs. Running Time');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'PSNR_vs_Time.png', '-dpng', '-r300');
...
xlabel('Equivalent Backprojection Number');
ylabel('SSIM');
title('SSIM Convergence Comparison');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'SSIM_vs_EBPN.png', '-dpng', '-r300');
...
xlabel('Time (seconds)');
ylabel('SSIM');
title('SSIM vs. Running Time');
legend show; grid on;
% --- 自动保存为 PNG ---
print(gcf, 'SSIM_vs_Time.png', '-dpng', '-r300');
...
% --- 自动保存为 PNG ---
print(gcf, 'Best_Images_and_Errors.png', '-dpng', '-r300');

%% ================= 8. 运行时间对比（控制台输出） =================
fprintf('========== Runtime Comparison (max EBPN = 200) ==========\n');
fprintf('EM    : %.2f sec\n', Result_EM.runtime);
...
fprintf('\n--- EBPN required to reach PSNR = %.1f dB ---\n', target_psnr);
fprintf('EM    : %.1f\n', ebpn_em_target);
fprintf('OS-EM : %.1f\n', ebpn_osem_target);
fprintf('SART  : %.1f\n', ebpn_sart_target);
fprintf('FBP   : 1 (fixed)\n');